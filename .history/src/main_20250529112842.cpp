...
bool testServerConnectivity() {
  ...
  // Test with the actual vehicle_data endpoint instead of root
  String response;
  bool connected = httpClient.get(VEHICLE_DATA_ENDPOINT, response);
  
  if (connected) {
    LOG_INFO(MODULE_HTTP, "✅ Server connectivity test PASSED");
    LOG_INFO(MODULE_HTTP, "✅ Vehicle data endpoint is accessible");
...
void handleSerialCommands() {
  if (SerialMon.available()) {
    ...
    if (cmd == "help") {
      ...
      LOG_INFO(MODULE_MAIN, "Endpoint: %s", VEHICLE_DATA_ENDPOINT);
    } else if (cmd == "reset") {
      ESP.restart();
