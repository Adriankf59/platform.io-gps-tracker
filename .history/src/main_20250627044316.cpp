 ...
 * - Fast error recovery
 * 
 * Versi: 6.1 - Fixed GPS/Modem/WebSocket Synchronization
 * Update: Mengatasi race condition dan memastikan sistem ready sebelum operasi
 */
...
enum SystemState {
  ...
  STATE_WAIT_GPS,             // Menunggu GPS fix (NEW)
...
struct SystemReadyFlags {
  ...
  unsigned long gpsFirstFixTime = 0;
...
// ----- GPS WAIT CONFIGURATION -----
const unsigned long GPS_WAIT_TIMEOUT = 90000;      // Max 90 detik tunggu GPS
const unsigned long GPS_MIN_SATELLITES = 4;        // Minimum satelit untuk fix
const float GPS_MAX_HDOP = 2.5;                    // Maximum HDOP untuk valid fix
const unsigned long GPS_LOG_INTERVAL = 5000;       // Log GPS status setiap 5 detik
...
void setup() {
  ...
  systemFlags.systemStartTime = millis();
  ...
  LOG_INFO(MODULE_MAIN, "Versi: 6.1 (GPS/Modem Sync Fixed)");
  ...
  LOG_INFO(MODULE_MAIN, "Menginisialisasi GPS...");
  gpsManager.begin();
  ...
  // Inisialisasi modem (hardware only, no connection yet)
  LOG_INFO(MODULE_MAIN, "Menginisialisasi hardware modem...");
  modemManager.begin();
  
  // Inisialisasi WebSocket manager
  wsManager.begin();
  ...
  // Set state awal untuk menunggu GPS
  ...
  LOG_INFO(MODULE_MAIN, "Setup selesai, menunggu GPS fix...");
}
...
void loop() {
  ...
  // Update GPS selalu (untuk mendapat fix)
  gpsManager.update();
  
  // Monitor kualitas sinyal dan network health hanya jika sistem ready
  ...
  // Update WebSocket dengan maintenance jika dalam mode continuous dan sistem ready
...
void handleWaitGpsState() {
  ...
  if (millis() - lastGpsStatusLog >= GPS_LOG_INTERVAL) {
    LOG_INFO(MODULE_GPS, "⏳ Waiting GPS fix... Elapsed: %lu s, Satellites: %d, HDOP: %.1f", 
             elapsed / 1000, gpsManager.getSatellites(), gpsManager.getHDOP());
    lastGpsStatusLog = millis();
  }
  
  // Check if GPS has enough satellites
  if (gpsManager.getSatellites() >= GPS_MIN_SATELLITES) {
    // Check if GPS fix is valid
    if (gpsManager.isValid() && gpsManager.getHDOP() <= GPS_MAX_HDOP) {
      ...
      LOG_INFO(MODULE_GPS, "🛰️ Satellites: %d, HDOP: %.1f", 
               gpsManager.getSatellites(), gpsManager.getHDOP());
      LOG_INFO(MODULE_GPS, "⏱️ Time to fix: %lu seconds", elapsed / 1000);
      
      // Pindah ke STATE_INIT untuk setup modem
      currentState = STATE_INIT;
      LOG_INFO(MODULE_SYS, "GPS ready, memulai inisialisasi modem...");
      return;
    }
  }
  ...
  if (elapsed >= GPS_WAIT_TIMEOUT) {
    LOG_WARN(MODULE_GPS, "⚠️ GPS fix timeout after %lu seconds", GPS_WAIT_TIMEOUT / 1000);
    LOG_WARN(MODULE_GPS, "Continuing with GPS not ready - will retry in background");
    
    // Continue anyway but mark GPS not ready
...
void handleInitState() {
  ...
  if (modemManager.setup()) {
    LOG_INFO(MODULE_SYS, "✅ Modem berhasil diinisialisasi");
    ...
    // Check GPS status again (mungkin sudah dapat fix saat setup modem)
    if (!systemFlags.gpsReady) {
      gpsManager.update();
      if (gpsManager.isValid()) {
        systemFlags.gpsReady = true;
        LOG_INFO(MODULE_GPS, "✅ GPS fix acquired during modem setup!");
      }
    }
    
    // Koneksi WebSocket jika mode continuous DAN setidaknya salah satu ready
    if (powerConfigs[currentPowerMode].wsContinuous && (systemFlags.gpsReady || systemFlags.modemReady)) {
      LOG_INFO(MODULE_WS, "Menghubungkan WebSocket...");
      if (wsManager.connect()) {
        LOG_INFO(MODULE_WS, "✅ WebSocket terhubung");
        
        // Wait for subscription
        LOG_INFO(MODULE_WS, "Menunggu WebSocket subscription...");
        unsigned long subWaitStart = millis();
        int waitCount = 0;
        
        while (wsManager.getState() != WS_SUBSCRIBED && 
               millis() - subWaitStart < WS_SUBSCRIPTION_TIMEOUT) {
          wsManager.update();
          
          if (++waitCount % 10 == 0) { // Log every second
            LOG_DEBUG(MODULE_WS, "Waiting for subscription... %d s", waitCount / 10);
          }
        ...
        if (wsManager.getState() == WS_SUBSCRIBED) {
          LOG_INFO(MODULE_WS, "✅ WebSocket subscribed successfully!");
          systemFlags.wsReady = true;
        } else {
          LOG_WARN(MODULE_WS, "⚠️ WebSocket subscription timeout");
          systemFlags.wsReady = false;
        }
      } else {
        LOG_WARN(MODULE_WS, "⚠️ Koneksi WebSocket gagal, akan dicoba lagi...");
        systemFlags.wsReady = false;
      }
    }
    ...
    // Proceed to operational even if not all systems ready
    currentState = STATE_OPERATIONAL;
    lastSuccessfulOperation = millis();
    
    // Reset GPS send timer for immediate first transmission if ready
    if (systemFlags.gpsReady && systemFlags.wsReady) {
      lastGpsSendTime = 0; // Force immediate send
      LOG_INFO(MODULE_SYS, "✅ System ready for GPS transmission!");
    }
    
  } else {
    LOG_ERROR(MODULE_SYS, "❌ Inisialisasi modem gagal, memulai reset");
    systemFlags.modemReady = false;
    modemManager.startReset();
    currentState = STATE_MODEM_RESET;
  }
}
...
void handleOperationalState() {
  ...
  // Check if GPS became ready (for case when GPS was not ready at startup)
  ...
  if (!systemFlags.gpsReady && gpsManager.isValid()) {
    ...
    LOG_INFO(MODULE_GPS, "✅ GPS fix acquired in operational state!");
    LOG_INFO(MODULE_GPS, "📍 Position: %.6f, %.6f", 
             gpsManager.getLatitude(), gpsManager.getLongitude());
  ...
  // Skip GPS operations if not ready
  if (!systemFlags.gpsReady) {
    if (!gpsReadyLogged) {
      LOG_WARN(MODULE_GPS, "GPS belum ready, waiting for fix...");
      LOG_INFO(MODULE_GPS, "Satellites: %d, HDOP: %.1f", 
               gpsManager.getSatellites(), gpsManager.getHDOP());
      gpsReadyLogged = true;
    }
    ...
    if (millis() - lastGpsLog > 10000) { // Every 10 seconds
      LOG_INFO(MODULE_GPS, "GPS status: %d sats, HDOP: %.1f", 
               gpsManager.getSatellites(), gpsManager.getHDOP());
      lastGpsLog = millis();
    }
    
    return; // Skip the rest of operational tasks
  }
  
  // Check WebSocket ready (reconnect if needed)
  if (!systemFlags.wsReady && powerConfigs[currentPowerMode].wsContinuous) {
    if (wsManager.getState() == WS_DISCONNECTED) {
      ...
      if (millis() - lastWsReconnect > 30000) { // Try every 30 seconds
        LOG_INFO(MODULE_WS, "Attempting WebSocket reconnection...");
        if (wsManager.connect()) {
  ...
  if (currentTime - lastGpsSendTime >= gpsInterval) {
    ...
    if (success) {
      ...
      if (!systemFlags.gpsReady) {
        LOG_ERROR(MODULE_GPS, "Send failed: GPS not ready");
      } else if (!systemFlags.modemReady) {
        LOG_ERROR(MODULE_GPS, "Send failed: Modem not ready");
      } else if (!systemFlags.wsReady) {
        LOG_ERROR(MODULE_GPS, "Send failed: WebSocket not ready");
      }
...
bool isSystemReady() {
  return systemFlags.gpsReady && 
         systemFlags.modemReady && 
         systemFlags.networkReady && 
         systemFlags.gprsReady &&
         (systemFlags.wsReady || !powerConfigs[currentPowerMode].wsContinuous);
}

void printSystemReadyStatus() {
  LOG_INFO(MODULE_SYS, "=== SYSTEM READY STATUS ===");
  LOG_INFO(MODULE_SYS, "GPS Ready: %s (First fix: %s)", 
           systemFlags.gpsReady ? "YES" : "NO",
           systemFlags.firstGpsFix ? "YES" : "NO");
  if (systemFlags.firstGpsFix) {
    unsigned long timeSinceStart = (systemFlags.gpsFirstFixTime - systemFlags.systemStartTime) / 1000;
    LOG_INFO(MODULE_SYS, "  Time to first fix: %lu seconds", timeSinceStart);
  }
  ...
  LOG_INFO(MODULE_SYS, "WebSocket Ready: %s", systemFlags.wsReady ? "YES" : "NO");
  LOG_INFO(MODULE_SYS, "Overall System: %s", isSystemReady() ? "READY ✅" : "NOT READY ⚠️");
  
  if (!isSystemReady()) {
    LOG_WARN(MODULE_SYS, "System will operate in degraded mode until all components ready");
  }
}
...
// ----- FUNGSI PENGIRIMAN DATA (UPDATED) -----
bool sendVehicleDataViaWebSocket() {
  // Pre-check all systems
  if (!systemFlags.gpsReady) {
    LOG_ERROR(MODULE_GPS, "❌ GPS belum ready, tidak dapat mengirim data");
    return false;
  }
  
  if (!systemFlags.modemReady || !systemFlags.gprsReady) {
    LOG_ERROR(MODULE_GPS, "❌ Modem/GPRS tidak ready");
    return false;
  }
  ...
  // Wait for SUBSCRIBED state if needed
  ...
  if (gpsManager.isValid()) {
    ...
    LOG_DEBUG(MODULE_GPS, "Satelit: %d, HDOP: %.1f", 
              gpsManager.getSatellites(), gpsManager.getHDOP());
    systemFlags.gpsReady = false; // Mark as not ready
  }
...
void handleSerialCommands() {
  if (SerialMon.available()) {
    ...
    if (cmd == "help") {
      ...
      // NEW: Show initialization status
      printSystemReadyStatus();
    } else if (cmd == "full") {
   ...
   // GPS commands
     ...
     LOG_INFO(MODULE_MAIN, "=== GPS STATUS ===");
     ...
     if (systemFlags.firstGpsFix) {
       unsigned long timeSinceStart = (systemFlags.gpsFirstFixTime - systemFlags.systemStartTime) / 1000;
       LOG_INFO(MODULE_MAIN, "First fix after: %lu seconds", timeSinceStart);
     }
   } else if (cmd == "forcegps") {
     // Force GPS ready for testing (dangerous!)
     LOG_WARN(MODULE_MAIN, "⚠️ FORCING GPS READY - FOR TESTING ONLY!");
     systemFlags.gpsReady = true;
     systemFlags.firstGpsFix = true;
     systemFlags.gpsFirstFixTime = millis();
   }
   // Debug commands
     ...
     delay(500);
     String response = modemManager.readATResponse(1000);
...
void printHelp() {
 ...
 SerialMon.println("init         - Tampilkan status inisialisasi sistem");
 ...
 SerialMon.println("\n=== GPS COMMANDS ===");
 SerialMon.println("gps          - Show detailed GPS status");
 SerialMon.println("forcegps     - Force GPS ready (testing only!)");
...
void printStatus() {
 ...
 if (!isSystemReady()) {
   ...
   if (!systemFlags.wsReady && powerConfigs[currentPowerMode].wsContinuous) 
     SerialMon.println("  - WebSocket not ready");
 }
 ...
 if (systemFlags.firstGpsFix) {
   unsigned long timeSinceStart = (systemFlags.gpsFirstFixTime - systemFlags.systemStartTime) / 1000;
   SerialMon.printf("First Fix    : After %lu seconds\n", timeSinceStart);
 }
