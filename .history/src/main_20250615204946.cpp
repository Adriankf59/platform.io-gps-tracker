...
void setup() {
  ...
  LOG_INFO(MODULE_MAIN, "Version: 2.5 (Vehicle Endpoint - No Port in URLs)"); // Version updated
  ...
  LOG_INFO(MODULE_MAIN, "Vehicle endpoint: %s%s", SERVER_HOST, VEHICLE_ENDPOINT);
  LOG_INFO(MODULE_MAIN, "Vehicle data endpoint: %s%s", SERVER_HOST, VEHICLE_DATA_ENDPOINT);
...
bool sendVehicleData() {
  ...
  // Send with retry - Use hostname and path separately (no port in URL)
...
bool testServerConnectivity() {
  ...
  if (connected) {
    ...
    LOG_INFO(MODULE_HTTP, "✅ Vehicle endpoint (%s%s) is accessible", SERVER_HOST, VEHICLE_ENDPOINT);
    LOG_INFO(MODULE_HTTP, "✅ Server %s is ready to receive data", SERVER_HOST);
