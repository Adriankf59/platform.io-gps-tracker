...
bool checkRelayStatus() {
  ...
  // Construct the API endpoint with filter for this vehicle and pending status
  String endpoint = "/items/commands?filter[vehicle_id][_eq]=1&filter[status][_eq]=pending";
  
  String response;
  if (!httpClient.get(endpoint.c_str(), response)) {
    LOG_ERROR(MODULE_RELAY, "Failed to get commands from API");
    return false;
  }
  
  LOG_DEBUG(MODULE_RELAY, "API Response: %s", response.c_str());
  ...
  if (!doc.containsKey("data") || doc["data"].size() == 0) {
    LOG_INFO(MODULE_RELAY, "No pending commands");
    return true;
  }
  
  // Process the most recent command
  JsonObject command = doc["data"][0];
  ...
  if (commandType == "ENGINE_OFF") {
    newRelayState = false;
  } else if (commandType == "ENGINE_ON") {
    newRelayState = true;
  } else {
  ...
  if (newRelayState != relayState) {
    LOG_INFO(MODULE_RELAY, "Changing relay state to: %s", newRelayState ? "ON" : "OFF");
...
void printStatus() {
  ...
  if (modemManager.isGprsConnected()) {
    ...
    LOG_INFO(MODULE_MAIN, "GPRS: Connected, Signal: %d 