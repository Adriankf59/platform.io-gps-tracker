...
void handleInitState() {
  ...
  // Wait for GPS to get initial fix before connecting
  LOG_INFO(MODULE_GPS, "Waiting for GPS fix before connecting...");
  unsigned long gpsWaitStart = millis();
  while (!gpsManager.isValid() && millis() - gpsWaitStart < 30000) {
    gpsManager.update();
    Utils::feedWatchdog();
    
    if (millis() - gpsWaitStart > 5000 && (millis() - gpsWaitStart) % 5000 < 100) {
      LOG_INFO(MODULE_GPS, "Still waiting for GPS... (sats: %d)", 
  ...
  if (gpsManager.isValid()) {
    LOG_INFO(MODULE_GPS, "✅ GPS fix acquired! Starting system...");
  } else {
    LOG_WARN(MODULE_GPS, "⚠️ No GPS fix after 30s, starting anyway...");
  }
...
void handleOperationalState() {
  ...
  if (forceSend || currentTime - lastGpsSendTime >= currentGpsInterval || gpsManager.hasNewFix()) {
    ...
    if (isOfflineMode && ENABLE_OFFLINE_BUFFERING) {
      // Only buffer if we have valid GPS data
      if (isValidGpsData()) {
        bufferGpsData();
      ...
      if (currentTime - offlineModeStartTime > 10000) {
        LOG_INFO(MODULE_SYS, "🔄 Attempting to reconnect...");
        if (wsManager.connect()) {
          LOG_INFO(MODULE_SYS, "✅ Reconnected! Sending buffered data...");
          isOfflineMode = false;
          sendBufferedData();
        } else {
          LOG_WARN(MODULE_SYS, "❌ Reconnection failed, staying in offline mode");
...
void updateGpsData() {
  ...
  GpsDataPoint currentPoint = {
    ...
    gpsManager.isSpeedValid() ? (float)gpsManager.getSpeed() : 0.0f,
    gps.course.isValid() ? (float)gps.course.deg() : lastValidPosition.heading,
    gpsManager.getSatellites(),
    millis(),
    true
  };
...
bool isValidGpsData() {
  if (!gpsManager.isValid()) {
    ...
    if (millis() - lastInvalidLog > 10000) {
      LOG_DEBUG(MODULE_GPS, "GPS not valid - waiting for fix (sats: %d)", 
                gpsManager.getSatellites());
  ...
  if (gpsManager.getSatellites() < MIN_SATELLITES_FOR_SEND) {
    static unsigned long lastSatLog = 0;
  ...
  if (gps.hdop.isValid() && gps.hdop.hdop() > MIN_HDOP_FOR_SEND) {
    static unsigned long lastHdopLog = 0;
    if (millis() - lastHdopLog > 10000) {
...
void bufferGpsData() {
  if (!isValidGpsData()) {
    static unsigned long lastInvalidLog = 0;
    if (millis() - lastInvalidLog > 5000) {
      LOG_DEBUG(MODULE_GPS, "Cannot buffer - GPS data invalid (sats:%d, valid:%s)", 
                gpsManager.getSatellites(), 
                gpsManager.isValid() ? "YES" : "NO");
      lastInvalidLog = millis();
    }
    return;
  }
  ...
  LOG_INFO(MODULE_GPS, "✅ GPS data buffered (count: %d/%d)", gpsBufferCount, GPS_DATA_BUFFER_SIZE);
}
...
void handleSerialCommands() {
  if (SerialMon.available()) {
    ...
    if (cmd == "help") {
      ...
      LOG_INFO(MODULE_MAIN, "Manual relay ON command");
      onRelayUpdate(true);
    } else if (cmd == "off") {
      LOG_INFO(MODULE_MAIN, "Manual relay OFF command");
      onRelayUpdate(false);
    } else if (cmd == "relay") {
      LOG_INFO(MODULE_MAIN, "=== RELAY STATUS ===");
      LOG_INFO(MODULE_MAIN, "Physical State: %s", relayState ? "ON" : "OFF");
      LOG_INFO(MODULE_MAIN, "Pin Level: %s", digitalRead(RELAY_PIN) ? "HIGH" : "LOW");
      LOG_INFO(MODULE_MAIN, "WebSocket Known State: %s", 
               wsManager.getLastKnownRelayState() ? "ON" : "OFF");
    } else if (cmd == "test_on") {
      LOG_INFO(MODULE_MAIN, "Testing relay callback with ON");
      wsManager.testRelayUpdate(true);
    } else if (cmd == "test_off") {
      LOG_INFO(MODULE_MAIN, "Testing relay callback with OFF");
      wsManager.testRelayUpdate(false);
    } else if (cmd == "gps") {
      ...
      LOG_INFO(MODULE_MAIN, "Last Known Relay: %s", 
               wsManager.getLastKnownRelayState() ? "ON" : "OFF");
    } else if (cmd == "connect") {
      wsManager.connect();
...
void printHelp() {
  ...
  LOG_INFO(MODULE_MAIN, "relay      - Show relay status details");
  LOG_INFO(MODULE_MAIN, "test_on/test_off - Test relay callback");
