...
void Utils::initWatchdog(uint32_t timeoutSeconds) {
  ...
  if (err != ESP_OK) {
    LOG_ERROR(MODULE_SYS, "Failed to initialize watchdog timer");
    return;
  }
  ...
  if (err != ESP_OK) {
    LOG_ERROR(MODULE_SYS, "Failed to add task to watchdog timer");
  } else {
    LOG_INFO(MODULE_SYS, "Watchdog timer initialized with %d second timeout", timeoutSeconds);
  }
}
...
int Utils::clearSerialBuffer(HardwareSerial& serial) {
  int clearedBytes = 0;
