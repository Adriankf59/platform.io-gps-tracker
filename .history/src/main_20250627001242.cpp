...
PowerModeConfig powerConfigs[3] = {
  // POWER_MODE_FULL - Optimized for minimum latency
  {
    .gpsInterval = 5000,                    // Reduced from 10000 to 5000ms
  ...
  {
    .gpsInterval = 60000,                   // Reduced from 300000 to 60000ms
    .wsKeepAliveInterval = 60000,           // Reduced from 120000
    .sleepDuration = 50000,                 // Reduced sleep time
    .gpsAlwaysOn = false,
    .wsContinuous = false,
    .relayEnabled = true,
  ...
  // POWER_MODE_EMERGENCY - Maximum power saving
  {
...
void handleOperationalState() {
  ...
  if (currentTime - lastGpsSendTime >= gpsInterval) {
    ...
    if (success) {
      ...
      if (performanceMetrics.consecutiveFailures >= CONSECUTIVE_SLOW_THRESHOLD) {
        LOG_WARN(MODULE_PERF, "🔧 Multiple failures detected, triggering optimization");
...
bool sendVehicleDataViaWebSocket() {
  ...
  if (gpsManager.isValid()) {
    // Get unix timestamp for compact transmission
    unsigned long unixTime = gpsManager.getUnixTimestamp();
    ...
    success = wsManager.sendVehicleDataCompact(
