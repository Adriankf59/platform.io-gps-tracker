...
bool checkRelayStatus() {
  ...
  if (commandType == "ENGINE_ON") {
    ...
    LOG_INFO(MODULE_RELAY, "✅ Relay turned ON successfully");
  } else if (commandType == "ENGINE_OFF") {
    LOG_INFO(MODULE_RELAY, "🔌 Executing ENGINE_OFF command - Turning relay OFF");
    digitalWrite(RELAY_PIN, RELAY_OFF);
    relayState = false;
    commandExecuted = true;
    LOG_INFO(MODULE_RELAY, "✅ Relay turned OFF successfully");
  } else {
  ...
  if (commandExecuted && !commandId.isEmpty()) {
    ...
    bool statusUpdated = updateCommandStatus(commandId, "acknowledge");
    if (statusUpdated) {
      LOG_INFO(MODULE_RELAY, "✅ Command %s executed and acknowledged successfully", commandId.c_str());
    } else {
      LOG_WARN(MODULE_RELAY, "⚠️ Command %s executed but failed to send acknowledge", commandId.c_str());
    }
