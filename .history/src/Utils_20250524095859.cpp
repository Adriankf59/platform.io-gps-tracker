...
void Utils::initWatchdog(uint32_t timeout_ms) {
  esp_task_wdt_config_t wdt_config = {
    ...
    .idle_core_mask = (1 << 0), // Core 0 mask
    .trigger_panic = true        // Reboot if timeout
  };
  
  // Disable existing watchdog if initialized
  esp_task_wdt_deinit();
  
  // Initialize new watchdog
  ...
  LOG_INFO(MODULE_SYS, "Watchdog initialized with %lu ms timeout", timeout_ms);
}
