 ...
 * 
 * Versi: 5.3 - Final Enhanced Edition
 * Update: Integrasi penuh dengan semua modul yang dioptimasi
 */
...
void setup() {
  ...
  LOG_INFO(MODULE_MAIN, "Versi: 5.3 (Final Enhanced Edition)");
...
void updateBatteryStatus() {
  ...
  if (millis() - lastBatteryCheck > 60000) { // Cek setiap menit
    ...
    LOG_DEBUG(MODULE_SYS, "Tegangan baterai: %.2f V (%s)", 
...
void printWebSocketStats() {
  ...
  LOG_INFO(MODULE_WS, "Total Messages: %lu", stats.totalMessages);
  LOG_INFO(MODULE_WS, "Bytes Sent: %lu KB (%.2f MB)", 
           stats.totalBytesSent / 1024,
           stats.totalBytesSent / 1048576.0);
  LOG_INFO(MODULE_WS, "Bytes Received: %lu KB (%.2f MB)", 
           stats.totalBytesReceived / 1024,
           stats.totalBytesReceived / 1048576.0);
  LOG_INFO(MODULE_WS, "Reconnect Count: %d", stats.reconnectCount);
  
  if (stats.connectionTime > 0 && wsManager.getState() != WS_DISCONNECTED) {
    LOG_INFO(MODULE_WS, "Connection Uptime: %s", 
             Utils::formatUptime(millis() - stats.connectionTime).c_str());
  }
  
  if (stats.lastMessageTime > 0) {
    LOG_INFO(MODULE_WS, "Last Message: %lu seconds ago", 
             (millis() - stats.lastMessageTime) / 1000);
  }
  
  // Calculate average message size
  if (stats.totalMessages > 0) {
    unsigned long avgSent = stats.totalBytesSent / stats.totalMessages;
    LOG_INFO(MODULE_WS, "Avg Message Size: %lu bytes", avgSent);
  }
}
...
void handleSerialCommands() {
  if (SerialMon.available()) {
    ...
    if (cmd == "help") {
      ...
      // Tampilkan WebSocket statistics
      printWebSocketStats();
    } else if (cmd == "wsreset") {
...
void printStatus() {
  ...
  // Tambah info WebSocket stats inline
  if (wsManager.getState() != WS_DISCONNECTED) {
    const WSStats& stats = wsManager.getStats();
