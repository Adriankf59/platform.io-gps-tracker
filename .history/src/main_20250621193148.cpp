// ========================================
// Main.cpp - Fixed HttpClient Compatibility
// ========================================
...
String getCurrentVehicleId() {
  ...
  // Build path for HTTP request (starts with /)
  String path = String(VEHICLE_ENDPOINT) + "?filter[gps_id][_eq]=" + String(GPS_ID);
  String response;
  
  LOG_DEBUG(MODULE_RELAY, "Looking up vehicle ID for GPS_ID: %s", GPS_ID);
  LOG_DEBUG(MODULE_RELAY, "HTTP GET path: %s", path.c_str());
  
  // Use HttpClient which handles HTTP (port 80) automatically
  bool success = httpClient.get(path.c_str(), response);
  
  if (success) {
    LOG_DEBUG(MODULE_RELAY, "✅ HTTP GET successful, response length: %d", response.length());
    ...
    if (!error && doc["data"].is<JsonArray>()) {
      ...
      if (vehicles.size() > 0) {
        ...
        if (vehicleId > 0) {
          LOG_INFO(MODULE_RELAY, "✅ Found vehicle_id: %d for GPS_ID: %s", vehicleId, GPS_ID);
          return String(vehicleId);
        }
      }
    } else {
      LOG_ERROR(MODULE_RELAY, "❌ JSON parse error: %s", error.c_str());
      LOG_DEBUG(MODULE_RELAY, "Response: %s", response.c_str());
    }
    
    LOG_ERROR(MODULE_RELAY, "❌ Vehicle not found for GPS_ID: %s", GPS_ID);
  } else {
    LOG_ERROR(MODULE_RELAY, "❌ HTTP GET request failed");
  }
...
bool sendRelayCommandStatus(const String& status) {
  ...
  // Build path for HTTP PATCH request
  String path = String(VEHICLE_ENDPOINT) + "/" + vehicleId;
  
  LOG_DEBUG(MODULE_RELAY, "HTTP PATCH path: %s", path.c_str());
  LOG_DEBUG(MODULE_RELAY, "Payload: %s", payload.c_str());
  
  // Use HttpClient which handles HTTP (port 80) automatically
  String response;
  bool success = httpClient.patch(path.c_str(), payload, response);
  ...
  if (success) {
    ...
    LOG_ERROR(MODULE_RELAY, "❌ HTTP PATCH request failed");
    LOG_DEBUG(MODULE_RELAY, "Response: %s", response.c_str());
    return false;
  }
}
...
void setup() {
  ...
  LOG_INFO(MODULE_MAIN, "Version: 3.5 (Compatible with HTTP Client)");
  LOG_INFO(MODULE_MAIN, "WebSocket URL: %s", WS_URL);
  LOG_INFO(MODULE_MAIN, "GPS ID: %s", GPS_ID);
  LOG_INFO(MODULE_MAIN, "Server Host: %s (HTTP)", SERVER_HOST);
...
void handleSerialCommands() {
  if (SerialMon.available()) {
    ...
    if (cmd == "help") {
      ...
      LOG_INFO(MODULE_MAIN, "Testing HTTP connection...");
      bool result = httpClient.testConnection();
      LOG_INFO(MODULE_MAIN, "HTTP test result: %s", result ? "SUCCESS" : "FAILED");
    } else if (cmd == "reset") {
      ESP.restart();
    } else {
...
void printHelp() {
  ...
  LOG_INFO(MODULE_MAIN, "http       - Test HTTP connection");
